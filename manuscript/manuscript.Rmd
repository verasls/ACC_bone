...
Bone responds to mechanical loading by altering its properties, such as density, geometry, and strength [@Turner2003; @Fonseca2014; @Santos2017]. Physical activity is a way of eliciting mechanical stimuli, and it is well established to improve bone health. However, there is no consensus in which characteristics of the physical activity-related mechanical stimulus are better associated with bone health benefits. Nevertheless, there is evidence that high-impact exercise, although performed scarcely throughout the day, may play an important role in inducing positive bone adaptations [@Vainionpaa2006; @Stiles2017].

But, even when knowing precisely which the best loading regimen is to improve bone health, the question of how to measure and monitor this load remains. Bone mechanical loading is usually measured in laboratory settings, using instruments such as force plates (FP) [@Medved2000]. Measurements in these instruments, that are confined to a laboratory space, are not applicable in controlling the loads that a person is routinely subjected to in free-living contexts. To surpass this issue, in the last few years, methods using wearable devices such as accelerometers have been proposed to measure mechanical loading, more precisely,  ground reaction force (GRF) [@Neugebauer2012; @Neugebauer2014; @Fortune2014; @Neugebauer2018; @Veras2020] and loading rate (LR) [@Fortune2014; @Veras2020]. These devices are light, small, can be used throughout days, and have a relatively affordable price. These characteristics make the accelerometers well-suited instruments to measure mechanical loading on a daily basis.

However, the validity of the accelerometer-predicted mechanical loading values depends on some factors, as the sample characteristics [@Welk2005]. In fact, due to this, several calibration studies have been done in several populations, such as children [@Neugebauer2012], adults [@Neugebauer2014; @Fortune2014], military personnel [@Neugebauer2018], and subjects with obesity [@Veras2020]. Another important factor that influences the validity of the predictions is the type of activities performed in the calibration study [@Bassett2012]. Until now, studies that calibrate accelerometers to predict mechanical loading variables have usually employed activities such as walking [@Fortune2014; @Neugebauer2018; @Veras2020], or walking and running [@Neugebauer2012; @Neugebauer2014]. Nevertheless, activities eliciting higher impact loads have not yet been tested.

Therefore, the objective of this study was to develop accelerometry-based mechanical loading prediction equations for high-impact activities in subjects from normal weight to severe obesity.
...
To visualize the agreement between the pGRF and pLR values measured by the FP and predicted by the models, a series of Bland-Altman plots were built for both the resultant and vertical vectors and for all accelerometer placements. Figure 2 shows the Bland-Altman plots for the pGRF (Panel A) and pLR (Panel B) resultant vector of hip-worn accelerometers. Both plots have shown good agreement levels, as at least `r ba_agreement`\% of the data points are within the limits of agreement and data tended to cluster around zero. Bland-Altman plots for the other accelerometer placements and vectors have shown the same patterns described previously, and can be found in the Supplemental Figure S2. Also, as could be observed by the one-sample *t*-tests results, none of the equations presented a bias different than zero (*p* > 0.05). Furthermore, all of the equations have shown a proportional bias, i.e., the magnitude of the values influences the prediction error. However, as the linear regressions presented a very small R^2^ (maximum value of `r myround(max(ba_regression_ankle_R2), 2)`) this effect is neglectable.
...
Although the accuracy of our equations showed to be beyond the established 8\% threshold, when applying a model not developed for jumping activities, we can see that the accuracy tremendously decreases, reaching values of MAPE around `r floor(mean(as.numeric(substr(tab2_df$MAPE, 1, nchar(tab2_df$MAPE) - 2))[1:4]) / 50) * 50`\% for pGRF prediction and around `r floor(mean(as.numeric(substr(tab2_df$MAPE, 1, nchar(tab2_df$MAPE) - 2))[5:8]) / 10) * 10`\% for pLR prediction as can be observed in Table 2. Furthermore, applying to our accelerometer-recorded jumping data a model that is not specific for this activity yields unreasonable results, such as the negative values for pGRF that can be observed in Figure 3, Panel A.

Finally, it is important to mention that an R package was developed [@Veras2021] to make it easier to apply all the necessary data processing steps to use the prediction equations. The package website (https://lveras.com/impactr) has all the installation instructions and documentation necessary to apply this method on your own data.

# Discussion {.unnumbered}

This study aimed to develop accelerometer-based mechanical loading prediction equations for jumps of diverse types and heights, testing different accelerometer placements and using both the resultant and vertical vectors. The results have shown that jumping pGRF prediction models based on accelerometry data have a lower accuracy compared to models developed for other activities, such as walking and running. Despite that, these models are the correct choice for this kind of activity eliciting hight levels of impact, as applying a model developed for lower impact activities in a sample similar to the current study yielded far worse results in terms of prediction accuracy. Also, as expected, the pLR prediction models performed poorly compared to the pGRF equations.

Bone is known to increase its tissue mass and enhance its micro-architecture arrangement in response to mechanical stimulation [@Turner2003; @Fonseca2014], particularly at higher magnitudes [@Vainionpaa2006; @Stiles2017]. This type of high-intensity impact activities have shown to be more associated with bone health improvements, even if they are usually performed in low quantities throughout the day. In fact, Vainionpää et al [@Vainionpaa2006] demonstrated that performing a relatively low number (<100) of pACC above 4.9*g* daily is positively associated with increases in bone mineral density. Usually, this acceleration magnitude can be achieved mostly by ballistic movements such as jumps [@Vainionpaa2006; @Rowlands2012]. Nevertheless, accelerometry-based mechanical loading regression models have mostly been validated with a set of activities, such as walking and running, that elicit relatively low acceleration magnitudes, usually ranging from approximately 1.2 to 6*g* [@Neugebauer2014; @Veras2020].
...
Mechanical loading prediction equations based on accelerometer data have already been proposed for several populations [@Neugebauer2012; @Neugebauer2018; @Veras2020] and using an accelerometer in different body placements [@Fortune2014; @Veras2020]. What these past few studies have in common is that they validated their models using locomotion activities, such as walking only or walking and running. The current study, by setting means to predict mechanical loading in several types and heights of jumps, broadens the application of such models, getting a step closer to the ability to accurately identify daily-living mechanical loading due to physical activity. This ability would allow the monitoring of loading in different contexts and to more fully understand the relationship between mechanical stimulation and its consequent effect on bone health.
