...
boxplot_ACC_res <- mechanical_load_data |>
...
boxplot_LR_res <- mechanical_load_data |>
  filter(acc_placement == "Lower back" & vector == "resultant") |>
...
boxplot_AR_res <- mechanical_load_data |>
  filter(vector == "resultant") |>
...
fig1 <- boxplot_GRF_res + theme(axis.text.x = element_blank()) +
  boxplot_LR_res + theme(axis.text.x = element_blank()) +
  boxplot_ACC_res +
  boxplot_AR_res +
